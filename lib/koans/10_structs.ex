defmodule Structs do
  ...
  koan "Use the put_in macro to replace a nested value" do
    ...
    assert put_in(airline.plane.maker, :airbus) == %Structs.Airline{name: "Southwest", plane: %Plane{passengers: 0, maker: :airbus}}
  end

  koan "Use the update_in macro to modify a nested value" do
    airline = %Airline{plane: %Plane{maker: :boeing, passengers: 200}}
    assert update_in(airline.plane.passengers, fn x -> x + 2 end) == %Structs.Airline{name: "Southwest", plane: %Plane{passengers: 202, maker: :boeing}}
  end
