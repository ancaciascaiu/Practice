...
begin
	...
	# start the consumer before the producer, we want to make sure the 
	# queue exists before we try to consume messages from it.
	#We're about to tell the server to deliver us the messages from 
	# the queue. Since it will push us messages asynchronously, 
	# we provide a callback that will be executed when RabbitMQ pushes 
	# messages to our consumer. This is what Bunny::Queue#subscribe does.
